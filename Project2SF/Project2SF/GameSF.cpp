...
int main()
{
	...
	while (window.isOpen())
	{
		...
		for (int i = 0; i < HEIGHT_MAP; i++)
			...
			{
				if (TileMap[i][j] == '0')  s_map.setTextureRect(IntRect(0, 0, 32, 32)); //åñëè âñòðåòèëè ñèìâîë 0, òî ðèñóåì 1é êâàäðàòèê
				if (TileMap[i][j] == '1')  s_map.setTextureRect(IntRect(32, 0, 32, 32));//åñëè âñòðåòèëè ñèìâîë 1, òî ðèñóåì 2é êâàäðàòèê
				if ((TileMap[i][j] == '2')) s_map.setTextureRect(IntRect(64, 0, 32, 32));//åñëè âñòðåòèëè ñèìâîë 2, òî ðèñóåì 3é êâàäðàòèê
				if ((TileMap[i][j] == '3')) s_map.setTextureRect(IntRect(96, 0, 32, 32));//è òä
		...
		window.draw(p.sprite);//рисую этот текст
